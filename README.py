...
class RevezamentoTurmaD:
    def __init__(self, root):
        ...
        self.dados_operadores = [
            {"nome": "Operador 1", "caminhao": "CAM-001", "inicio_janta": "06:00 PM", "fim_janta": "06:30 PM"},
            {"nome": "Operador 2", "caminhao": "CAM-002", "inicio_janta": "06:15 PM", "fim_janta": "06:45 PM"},
            {"nome": "Operador 3", "caminhao": "CAM-003", "inicio_janta": "11:30 PM", "fim_janta": "12:30 AM"},
        ]
    ...
    def parse_time(self, time_str):
        """Converte string de tempo no formato 'HH:MM AM/PM' para objeto datetime"""
        try:
            return datetime.strptime(time_str, "%I:%M %p")
        except ValueError:
            # Se falhar, tenta sem o AM/PM (para compatibilidade com versões anteriores)
            try:
    ...
    def format_time(self, dt):
        """Formata objeto datetime para string no formato 'HH:MM AM/PM'"""
    ...
    def adicionar_operador(self):
        ...
        novo_operador = {
            "nome": "Novo Operador",
            "caminhao": "CAM-000",
            "inicio_janta": self.format_time(agora),
            "fim_janta": self.format_time(agora + timedelta(minutes=30))
        }
    ...
    def update_timers(self):
        ...
        for item in self.tree.get_children():
            ...
            try:
                inicio_janta = self.parse_time(inicio_str)
                fim_janta = self.parse_time(fim_str)
                
                if inicio_janta is None or fim_janta is None:
                    continue
                
                # Ajustar para a data atual
                ...
                if fim_janta < inicio_janta:
                    if agora.hour >= 12:  # Se for PM, assume que é no mesmo dia
                        fim_janta = fim_janta.replace(day=agora.day + 1)
                    else:  # Se for AM, assume que o início foi no dia anterior
                        inicio_janta = inicio_janta.replace(day=agora.day - 1)
